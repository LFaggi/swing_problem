...
class NeuralAgent:
    def __init__(self):
        self.xi = np.ones(4)
        self.omega = 0.1 * np.random.rand(9)

        self.p_xi = np.ones(4)
        self.p_omega = np.ones(9)

        self.agent_states_and_costates = [self.xi,self.omega,self.p_xi,self.p_omega]
    ...
    def read_env_parameters(self,env_parameters):
        ...
        self.slack_xi = env_parameters[3]
        self.slack_omega = env_parameters[4]
        self.m_v = env_parameters[5]
        self.m_xi = env_parameters[6]


    def compute_activations(self):
        activations = np.zeros(4)
        activations[0] = self.xi[0] * self.omega[0] * self.theta[0] + self.xi[1] * self.omega[4] * self.theta[4] +  self.xi[2] * self.omega[5] * self.theta[5]
        activations[1] = self.xi[0] * self.omega[4] * self.theta[4] + self.xi[1] * self.omega[1] * self.theta[1] +  self.xi[3] * self.omega[7] * self.theta[7]
        activations[2] = self.xi[0] * self.omega[5] * self.theta[5] + self.xi[2] * self.omega[2] * self.theta[2] + self.xi[3] * self.omega[8] * self.theta[8]
        activations[3] = self.xi[1] * self.omega[7] * self.theta[7] + self.xi[2] * self.omega[8] * self.theta[8] + self.xi[3] * self.omega[3] * self.theta[3]
        return activations

    def compute_costate_temp_der(self,input):
        g_p_xi = np.zeros(4)
        g_p_omega = np.zeros(9)
        g_p_xi[0] = -self.dissipation_factor * ((self.xi[0]-input)-(self.xi[3]-self.xi[0])-self.m_xi[0])



    def update_states_costates(self, env_parameters, input):
        self.read_env_parameters(env_parameters)

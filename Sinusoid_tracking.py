...
phi = 1
m_omega = torch.ones(n_neurons, n_neurons)
m_xi = torch.ones(n_neurons)


#Initialization of states and costates
torch.manual_seed(3)

xi = torch.zeros(n, n_neurons)
omega = torch.rand(n, n_neurons, n_neurons)
p_xi = 0.1 * torch.ones(n, n_neurons)
p_omega = 0.1 * torch.ones(n, n_neurons, n_neurons)
...
#Prime derivative of the activation function of the neurons
...
class NeuralModel(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_layer4 = torch.nn.Linear(self.hidden, self.n_neurons)
...
def backward_learning(xi_0, p_xi_0, omega_0, p_omega_0, t_index, net, window):
    ...
    for window_index in range(window-1):
        for i in range(n_neurons):
            ...
            for j in range(n_neurons):
                omega_train[window - window_index - 2][i][j] = omega[window - window_index - 1][i][j] - dt * (-p_omega_train[window - window_index - 1][i][j] / (phi * m_omega[i][j]))
...
if __name__ == "__main__":
    ...
    p_xi_f = torch.zeros(n, n_neurons)
    omega_f = torch.rand(n, n_neurons, n_neurons)
    p_omega_f = torch.zeros(n, n_neurons, n_neurons)
    ...
    for t in range(len(t_array)-1):
        ...
        if torch.norm(xi_f[t+1])>threshold or torch.norm(omega_f[t+1])>threshold or torch.norm(p_xi_f[t])>threshold or torch.norm(p_omega_f[t])>threshold:
            xi_f[t+1] = torch.zeros(n_neurons)
            p_xi_f[t] = torch.zeros(n_neurons)
            omega_f[t + 1] = torch.zeros(n_neurons, n_neurons)
            p_omega_f[t + 1] = torch.zeros(n_neurons, n_neurons)
            print("Reset!")
        if t % 1000 == 0:
...
#------------------------------------------------ PLOTS ---------------------------------------------------------------
    ...
    for i in range(n_neurons):
        plt.plot(t_array, xi_f[:,i], label=f'xi_{i}')
        plt.plot(t_array, p_xi_f[:,i], label=f'p_xi_{i}')
    ...
    for i in range(n_neurons):
        for j in range(n_neurons):
            plt.plot(t_array, omega_f[:,i,j], label=f'omega_{i}{j}')
            plt.plot(t_array, p_omega_f[:, i, j], label=f'p_omega_{i}{j}')
